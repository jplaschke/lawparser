        ...
        try:
            if link.has_attr('href'):
                ...
                if (link['href'].endswith("ZS.html")):
                    ...
                    scrapeWebSyllabus(textStr, link['href'],topic,url)
                    #outcsv.close()
                    #sys.exit(-1)
        except:
            print (sys.exc_info()[0])
...
def scrapeWebSyllabus(casetitle, url, topic,topurl):
    global outcsv
    ...
    for link in soup.findAll('a'):
        ...
        try:
            if link.has_attr('href'):
                ...
                if (re.search("ZO[1-9]*", link['href'], flags=0) != None) or (link['href'].endswith("_ZO")):
                    print( "OPINION FOUND = "+"link text: "+clean+"\t\t ***: "+link['href'])
                    outcsv.write("*opinion:*|code text|code_url|"+url+"|"+topic+"|opinion|"+clean+"|"+link['href']+"|"+topurl+"\n")
                elif (re.search("ZC[1-9]*", link['href'], flags=0) != None) or (link['href'].endswith("_ZC")):
                    print( "Concurrence FOUND = "+"link text: "+clean+"\t\t ***: "+link['href'])
                    outcsv.write("*opinion:*|code text|code_url|"+url+"|"+topic+"|concurrence|"+clean+"|"+link['href']+"|"+topurl+"\n")
                elif (re.search("ZD[1-9]*", link['href'], flags=0) != None) or (link['href'].endswith("_ZD")):
                    print( "Dissent FOUND = "+"link text: "+clean+"\t\t ***: "+link['href'])
                    outcsv.write("*opinion:*|code text|code_url|"+url+"|"+topic+"|dissent|"+clean+"|"+link['href']+"|"+topurl+"\n")
                elif (re.search("ZX[1-9]*", link['href'], flags=0) != None) or (link['href'].endswith("_ZD")):
                    print( "Concurinpart FOUND = "+"link text: "+clean+"\t\t ***: "+link['href'])
                    outcsv.write("*opinion:*|code text|code_url|"+url+"|"+topic+"|concur in part|"+clean+"|"+link['href']+"|"+topurl+"\n")
                elif (re.search("[1-9]+\s*U\.\s*S\.\s*[1-9]+", clean, flags=0) != None):
                    print( "Citation FOUND = "+"link text: "+clean+"\t\t ***: "+link['href'])
                    clean = clean.strip()
                    cite_url = link['href'].strip()
                    url = url.strip()
                    #outcsv.write(clean+"|"+cite_url+"|"+url+"|"+topic+"\n")
                elif (re.search("[1-9]+\s*U\.\s*S\.\s*C\.[\(\)a-z0-9\s]*", clean, flags=0) != None):

                    print( "Code FOUND = "+"link text: "+str(clean)+"\t\t ***: "+str(link['href']))
                    clean = clean.strip()
                    code_url = link['href'].strip()
                    url = url.strip()
                    outcsv.write("Code|"+clean+"|"+code_url+"|"+url+"|"+topic+"|opinion type|option text|"+topurl+"\n")
...
def main():
   ...
   # url = "/supct/search/display.html?terms=bankruptcy&url=/supct/html/10-179.ZS.html"
    ...
    outcsv = open("./scotus_code.csv","w")
    outcsv.write("cite name | cite url | case url\n")
